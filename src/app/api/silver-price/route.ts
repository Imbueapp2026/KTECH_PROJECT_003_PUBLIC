/**
 * GET /api/silver-price — public endpoint for current silver price
 * Returns the current silver price per gram with comparison data
 */
...
export async function GET(req: Request) {
  ...
  try {
    ...
    // Get the most recent silver price
    ...
    if (error || !data) {
      ...
      const response = Response.json({
        price_per_gram: 90, // Fallback silver price (e.g. ₹90/g)
    ...
    const errorResponse = serverError("Failed to fetch silver price");
